...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_agent", Command_Main);
...
public Action Command_Main(client, args)
{
	...
	SetMenuTitle(menu, "Alege Agentul pentru:");
	
	AddMenuItem(menu, "", "Counter-Terrorist");
	AddMenuItem(menu, "", "Terrorist");
...
void OpenAgentsMenu(int client)
{
	...
	SetMenuTitle(menu, "Alege Modelul Agentului:");
	
	AddMenuItem(menu, "", "Modelul Default al Agentului");
		...
		case MenuAction_Select:
		{
			switch(param2)
			{
				case 0:{
					...
					PrintToChat(param1, "Nu ai un model de Agent pe tine acum!");
	...
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if(cv_instant.BoolValue)
				PrintToChat(param1, "Modelul Agentului ales!");
			else
				PrintToChat(param1, "Modelul Agentului ales! O sa apara pe tine in urmatoarea runda!");
			...
			if(cv_instant.BoolValue && IsPlayerAlive(param1) && GetClientTeam(param1) == g_iTeam[param1])
			{
				if(cv_noOverwritte.BoolValue)
				{
					...
					{
						PrintToChat(param1, "Ai deja un model custom pe tine, dezactiveaza acel model custom pentru a alege un model de Agent.");
						return;
					}
				}
...
public Action Timer_ApplySkin(Handle timer, int id)
{
	...
	if(cv_noOverwritte.BoolValue)
	{
		...
		if(StrContains(dmodel, "models/player/custom_player/legacy/") == -1)
		{
			PrintToChat(client, "Ai deja un model custom pe tine, dezactiveaza acel model custom pentru a alege un model de Agent.");
			return;
		}
	}
