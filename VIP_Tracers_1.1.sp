...
public OnPluginStart() 
{
	...
	SetMenuTitle(g_hColorsMenu, "Culoare Tracer:\n \n");
...
public OnMapStart()
{
	...
	if(sBuffer[0] == 0)
    {
		AddMenuItem(g_hColorsMenu, "", "Fara culori", ITEMDRAW_DISABLED);  
    }
...
public Handler_ColorsMenu(Handle:hMenu, MenuAction:action, iClient, Item)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			VIP_PrintToChatClient(iClient, "\x03Ati schimbat culoarea de la tracer in \x04%s", sColorName);
