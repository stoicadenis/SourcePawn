...
public Action Command_hud(int client, int args) 
{
	...
	{
		PrintToChat(client, " ★ \x02HUD-ul este acum oprit!");
		g_bIsHudEnabled[client] = false;
		SetClientCookie(client, g_hHUDv2Cookie, "0");
	}
	else
	{
		PrintToChat(client, " ★ \x04HUD este acum pornit!");
		g_bIsHudEnabled[client] = true;
...
public Action TIMER(Handle timer, any client)
{
	...
	char sTime[255], szTime[255], iMessage1[32], iMessage2[32], iMessage3[32], MapTimeLeft[128];
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(g_bIsHudEnabled[i] && IsClientValid(i))
		{
			...
			Format(iBuffer, sizeof(iBuffer),"%s\n%s\nPlayeri: %d/%d\nTimeleft: %s\nCeas: %s",iMessage1, iMessage2, clientCount, g_cvarSlots.IntValue, MapTimeLeft, szTime);
